...
namespace Game {
    public static class Math {
        ...
        ///     another range [<paramref name="minB" />, <paramref name="maxB" />]
        /// </summary>
        /// <param name="_">This exists only to allow two methods with the same signature since this method is a float extension</param>
        ...
        /// <summary>
        /// Checks if <paramref name="comp"/> is between <paramref name="min"/> (inclusive) and <paramref name="max"/> (inclusive).
        /// </summary>
        /// <param name="comp">The value to compare</param>
        /// <param name="min">The minimum value to compare against (inclusive)</param>
        /// <param name="max">The maximum value to compare against (inclusive)</param>
        /// <returns><value>true</value> if <para>comp</para>&gt;=<para>min</para> and <para>comp</para>&lt;=<para>max</para></returns>
        public static bool BetweenInclusive<TA>(this TA comp, TA min, TA max) where TA : IComparable {
            return comp.CompareTo(min) >= 0 && comp.CompareTo(max) <= 0;
        }
    }
}